...
from NewBoard.settings import DEFAULT_FROM_EMAIL
...
def sign_up(request):
    if not request.user.is_authenticated:
        if request.POST:
            ...
            return render(request, 'sign/signup.html', {'form': RegistrationForm()})
    else:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if not profile.user.is_active:
                    ...
                    form.add_error(None, 'Неизвестный или неактивированный аккаунт')
                return render(request, 'sign/activate.html', {'form': form})
            else:
                form.add_error(None, 'Неверная форма ввода')
                return render(request, 'sign/activate.html', {'form': form})
        else:
